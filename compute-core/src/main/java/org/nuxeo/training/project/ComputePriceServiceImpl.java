...
public class ComputePriceServiceImpl extends DefaultComponent implements ComputePriceService {
    ...
    public void registerContribution(Object contribution, String extensionPoint, ComponentInstance contributor) {
        ...
        // Framework.getRuntime().getWarnings().add("in registerContribution for
        // pricing");
        ...
        if ("pricing".equals(extensionPoint)) {
            log.info("pricing OK");
            // Framework.getRuntime().getWarnings().add("in registerContribution
            // pricing OK");
        }
    ...
    public float computePrice(Product prod) {
        ...
        if (customization == null) {
            ...
            /*
             * for (String price: prices) { String item = price.substring(price.indexOf("item=\"")+"item=\"".length());
             * if (item.length() == 0) { continue; } item = item.substring(0, item.indexOf('"')); String unitpricestr =
             * price.substring(price.indexOf("unit=\"")+"unit=\"".length()); if (unitpricestr.length() == 0) { continue;
             * } unitpricestr = unitpricestr.substring(0, item.indexOf('"')); float unitprice =
             * Float.parseFloat(unitpricestr); if (prod.getName().indexOf(item) != 0) { return unitprice; } }
             */
    ...
    /*
     * @Override public void registerExtension(Extension extension) { // Add some logic here to handle contributions
     * log.error("in registerExtension; testing pricing"); Framework.getRuntime().getWarnings().
     * add("in registerExtension for pricing - 1"); if ("pricing".equals(extension.getExtensionPoint())) {
     * log.error("pricing OK"); Framework.getRuntime().getWarnings(). add("in registerExtension for pricing - 2"); }
     * Object[] os = extension.getContributions(); if (os != null && os.length > 0) {
     * Framework.getRuntime().getWarnings().add("YES"); for (Object o: os) {
     * Framework.getRuntime().getWarnings().add("o.class="+o.getClass().toString ()); if (o instanceof java.lang.String)
     * { customization = (String) o; Framework.getRuntime().getWarnings().
     * add("in registerExtension customization ="+customization); } else { Framework.getRuntime().getWarnings().
     * add("in registerExtension not a String"); } } } else {
     * Framework.getRuntime().getWarnings().add("os is null or has length 0"); } }
     * @Override public void unregisterExtension(Extension extension) { // Logic to do when unregistering any
     * contribution // customization = null; }
     */
