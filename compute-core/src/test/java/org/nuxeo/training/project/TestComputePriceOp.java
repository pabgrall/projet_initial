...
@Features({PlatformFeature.class, AutomationFeature.class, CoreFeature.class})
// @RepositoryConfig(init = DefaultRepositoryInit.class, cleanup = Granularity.METHOD)
// @LocalDeploy("org.nuxeo.training.project.computeprice-core:computeprice-extension.xml")
// @PartialDeploy(bundle = "studio.extensions.pabgrall-SANDBOX", features = { TargetExtensions.Automation.class })
...
public class TestComputePriceOp {
    ...
    public void shouldCallTheOperation() throws OperationException {
        ...
        DocumentModel doc = (DocumentModel) automationService.run(ctx, ComputePrice.ID);
	...
	*/
    ...
    public void shouldCallWithParameters() throws OperationException {
        ...
        DocumentModel doc = createDocType(session, path, "myProduct", "Products");
        if (doc!=null) {
        ...
        doc = (DocumentModel) automationService.run(ctx, "Document.ComputePriceOp"/* , params*/);
        assertNotNull(doc);
        assertEquals(path+"/Products", doc.getPathAsString());
        assertEquals("StringProperty(product_schema:price*=2.4)", doc.getProperty("price").toString());
        
        
        DocumentModelList dml = new DocumentModelListImpl();
        
        DocumentModel doc2 = createDocType(session, path, "myProduct2", "Products");
        ...
        dml = (DocumentModelList) automationService.run(ctx, "Document.ComputePriceOp"/*, params*/);
        assertNotNull(dml);
        assertEquals(dml.totalSize(), 2);
        assertEquals(dml.get(0).getPropertyValue("price"), (new Float(4.8f)).toString());
        assertEquals(((String)(dml.get(1).getPropertyValue("price"))).substring(0,3), (new Float(3.6f)).toString());
    }
    
    private DocumentModel createDocType(CoreSession session, String path, String name, String type) {
